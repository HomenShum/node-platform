...
test("fresh-agent recorder preserves process fields and prohibits routine reprompts", async () => {
  ...
  assert.match(harness, /lower-cost profile requires --agentModel/);
  assert.match(harness, /claude-code/);
  assert.match(harness, /--no-session-persistence/);
  assert.match(harness, /isolatedAgentEnvironment/);
  assert.match(harness, /baseEnv: isolatedAgentEnvironment\(agentDriver\)/);
  ...
  assert.match(harness, /installedPackageMatchesArchive/);
