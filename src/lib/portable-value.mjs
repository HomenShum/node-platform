...
export function normalizePortableValue(value, label = "value", options = {}) {
  ...
  function visit(candidate, path, depth) {
    ...
    if (typeof candidate === "string") return requirePortableString(candidate, path);
    if (typeof candidate === "number") {
      if (!Number.isFinite(candidate)) throw new TypeError(`${path} must be a portable value; non-finite numbers are not supported`);
    ...
    if (candidate === undefined) throw new TypeError(`${path} must be a portable value; undefined is not supported`);
    if (typeof candidate === "bigint") throw new TypeError(`${path} must be a portable value; BigInt is not supported`);
    if (typeof candidate === "function") throw new TypeError(`${path} must be a portable value; functions are not supported`);
    if (typeof candidate === "symbol") throw new TypeError(`${path} must be a portable value; symbols are not supported`);
    if (typeof candidate !== "object") throw new TypeError(`${path} must be a portable value`);
    if (depth > maxNestingDepth) {
      throw new TypeError(`${path} must be a portable value; nesting exceeds ${maxNestingDepth} levels`);
    }
    if (ancestors.has(candidate)) throw new TypeError(`${path} must be a portable value; cyclic references are not supported`);
    ...
    try {
      if (Array.isArray(candidate)) {
        ...
        if (symbolKeys.length > 0) throw new TypeError(`${path} must be a portable value; symbol keys are not supported`);
...
export function normalizeStageDefinitions(stages) {
  ...
  if (!Array.isArray(portableStages)) throw new Error("run stages are required");
  const seen = new Set();
  return portableStages.map((candidate, index) => {
    ...
    if (seen.has(id)) throw new Error(`stage ids must be unique: ${id}`);
    seen.add(id);
...
export function stageDefinitionsMatch(currentStages, requestedStages) {
  if (!Array.isArray(currentStages) || currentStages.length !== requestedStages.length) return false;
