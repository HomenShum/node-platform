...
function canonicalizePortable(value: unknown): string {
  if (Array.isArray(value)) return `[${value.map(canonicalizePortable).join(",")}]`;
  ...
  if (encoded === undefined) throw new Error("NodeKit hashes do not accept undefined values");
  return encoded;
}
