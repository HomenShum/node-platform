...
const SCHEMA = {
  approval: "nodekit.approval/v1" as const,
  artifact: "nodekit.artifact/v1" as const,
  case: "nodekit.case/v1" as const,
  event: "nodekit.caseflow-event/v1" as const,
  exception: "nodekit.exception/v1" as const,
  proposal: "nodekit.proposal/v1" as const,
  receipt: "nodekit.receipt/v2" as const,
  run: "nodekit.run/v1" as const,
};
...
function actorOrSystem(actor?: Actor): Actor {
...
function requireActiveRun(run: Doc<"runs">): void {
  if (run.status === "active") return;
  if (TERMINAL_RUN_STATUSES.has(run.status)) throw new Error(`run is terminal: ${run.status}`);
  throw new Error(`run is not active: ${run.status}`);
}

function requireNonTerminalRun(run: Doc<"runs">): void {
  if (TERMINAL_RUN_STATUSES.has(run.status)) throw new Error(`run is terminal: ${run.status}`);
}

function requireTransportHash(expected: string, actual: string, label: string): void {
  if (!/^[a-f0-9]{64}$/.test(expected)) throw new Error(`${label} must be a lowercase SHA-256 digest`);
  if (expected !== actual) throw new Error(`${label} does not match the transported portable value`);
}
...
async function requireScoped<T extends TableNames>(
  ctx: DatabaseCtx,
  table: T,
  rawId: string,
  scopeKey: string,
  ...
  let record: Doc<TableNames> | null = null;
  if (table === "cases") record = await ctx.db.query("cases").withIndex("by_scope_id", (q) => q.eq("scopeKey", scopeKey).eq("caseId", rawId)).unique();
  else if (table === "runs") record = await ctx.db.query("runs").withIndex("by_scope_id", (q) => q.eq("scopeKey", scopeKey).eq("runId", rawId)).unique();
  else if (table === "artifacts") record = await ctx.db.query("artifacts").withIndex("by_scope_id", (q) => q.eq("scopeKey", scopeKey).eq("artifactId", rawId)).unique();
  else if (table === "proposals") record = await ctx.db.query("proposals").withIndex("by_scope_id", (q) => q.eq("scopeKey", scopeKey).eq("proposalId", rawId)).unique();
  else if (table === "approvals") record = await ctx.db.query("approvals").withIndex("by_scope_id", (q) => q.eq("scopeKey", scopeKey).eq("approvalId", rawId)).unique();
  else if (table === "exceptions") record = await ctx.db.query("exceptions").withIndex("by_scope_id", (q) => q.eq("scopeKey", scopeKey).eq("exceptionId", rawId)).unique();
  else if (table === "receipts") record = await ctx.db.query("receipts").withIndex("by_scope_id", (q) => q.eq("scopeKey", scopeKey).eq("receiptId", rawId)).unique();
  else if (table === "timelineEvents") record = await ctx.db.query("timelineEvents").withIndex("by_scope_id", (q) => q.eq("scopeKey", scopeKey).eq("eventId", rawId)).unique();
  else throw new Error(`unsupported scoped table: ${String(table)}`);
  if (record === null) throw new Error(`${label} not found: ${rawId}`);
  return record as unknown as Doc<T>;
}

function toCase(record: Doc<"cases">) {
  return {
...
function toRun(record: Doc<"runs">) {
  return {
    caseId: record.caseId,
    createdAt: record.createdAt,
...
function toProposal(record: Doc<"proposals">) {
  return {
...
function toApproval(record: Doc<"approvals">) {
  return {
...
function toException(record: Doc<"exceptions">) {
  return {
    code: record.code,
...
function toReceipt(record: Doc<"receipts">) {
  return {
...
function toEvent(record: Doc<"timelineEvents">) {
  return {
    actor: record.actor,
...
async function artifactOutput(ctx: DatabaseCtx, record: Doc<"artifacts">) {
  const versions = await ctx.db
    .query("artifactVersions")
    .withIndex("by_scope_artifact_version", (q) =>
      q.eq("scopeKey", record.scopeKey).eq("artifactId", record.artifactId),
    )
    .collect();
  return {
    artifactId: record.artifactId,
...
async function emit(
  ...
  const previous = await ctx.db
    ...
    .withIndex("by_scope_aggregate", (q) =>
      q.eq("scopeKey", scopeKey).eq("aggregateType", aggregateType).eq("aggregateId", aggregateId),
    )
    .order("desc")
    .first();
...
async function findIdempotentEvent(
  ...
  operation: string,
  request: unknown,
): Promise<{ event: Doc<"timelineEvents"> | null; key: string | undefined; requestHash: string }> {
  const requestHash = contentHash({ operation, request });
  if (idempotencyKey === undefined) return { event: null, key: undefined, requestHash };
  const key = requireTrimmedText(idempotencyKey, "idempotencyKey");
  const event = await ctx.db
    .query("timelineEvents")
    .withIndex("by_scope_idempotency", (q) =>
      q.eq("scopeKey", scopeKey).eq("idempotencyKey", key),
    )
    .first();
...
export const createCase = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.scopeKey.length === 0) throw new Error("scopeKey is required");
    if (title.length === 0) throw new Error("case title is required");
    if (primaryJob.length === 0) throw new Error("case primaryJob is required");
    const now = timestamp();
...
export const updateCaseInput = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const title = args.title?.trim();
    if (primaryJob.length === 0) throw new Error("case primaryJob is required");
    if (title !== undefined && title.length === 0) throw new Error("case title cannot be empty");
    if (primaryJob === record.primaryJob && (title === undefined || title === record.title)) return toCase(record);
...
export const startRun = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const normalizedStages = normalizeStageDefinitions(args.stages) as Array<{
      id: string;
      label: string;
      owner: string;
...
export const enterStage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (idempotency.event !== null) {
      ...
      return idempotency.event.idempotencyResult as ReturnType<typeof toRun>;
    }
    ...
    const stages = run.stages.map((stage, index) => ({
      ...stage,
      status: index < targetIndex ? ("completed" as const) : index === targetIndex ? ("active" as const) : ("pending" as const),
    }));
...
export const createArtifact = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (idempotency.event !== null) {
      if (idempotency.event.idempotencyResult === undefined) throw new Error("idempotency result is missing");
      return idempotency.event.idempotencyResult as Awaited<ReturnType<typeof artifactOutput>>;
    }
    await requireScoped(ctx, "cases", args.caseId, args.scopeKey, "case");
    const run = await requireScoped(ctx, "runs", args.runId, args.scopeKey, "run");
    if (run.caseId !== args.caseId) throw new Error("run does not belong to case");
    requireActiveRun(run);
    const now = timestamp();
...
export const createProposal = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!Number.isInteger(args.baseVersion) || args.baseVersion < 1) throw new Error("baseVersion must be a positive integer");
    ...
    if (idempotency.event !== null) {
      if (idempotency.event.idempotencyResult === undefined) throw new Error("idempotency result is missing");
      return idempotency.event.idempotencyResult as ReturnType<typeof toProposal>;
    }
...
export const decideProposal = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    let nextStatus: "accepted" | "conflicted" | "rejected" = args.decision;
    if (args.decision === "accepted" && proposal.baseVersion !== artifact.canonicalVersion) nextStatus = "conflicted";
...
type TerminalReceiptStatus = "cancelled" | "completed" | "failed_safely";
  ...
  reason?: string,
    ...
    if (run.status === status) {
      const receipt = await ctx.db
        ...
        .first();
      if (receipt === null) throw new Error(`${status} run is missing its receipt`);
      const runEvents = await ctx.db
        .query("timelineEvents")
        .withIndex("by_scope_aggregate", (q) =>
          q.eq("scopeKey", args.scopeKey).eq("aggregateType", "run").eq("aggregateId", args.runId),
        )
        .collect();
    ...
    if (TERMINAL_RUN_STATUSES.has(run.status)) throw new Error(`run is terminal: ${run.status}`);
    if (status === "completed") requireActiveRun(run);
    ...
    if (status === "completed") {
      ...
      if (exceptions.some((entry) => entry.status === "open")) throw new Error("run has unresolved exceptions");
    ...
    const rawArtifactBindings = await Promise.all(runArtifacts.map(async (artifact) => {
      const canonicalVersion = await ctx.db
        ...
        .first();
      if (canonicalVersion === null) throw new Error(`artifact ${artifact.artifactId} is missing its canonical version`);
      return {
    ...
    const proposalGroups = await Promise.all(rawArtifactIds.map((artifactId) =>
      ctx.db.query("proposals").withIndex("by_scope_artifact", (q) =>
        q.eq("scopeKey", args.scopeKey).eq("artifactId", artifactId),
      ).collect(),
    ));
    ...
    const approvalGroups = await Promise.all(rawProposalIds.map((proposalId) =>
      ctx.db.query("approvals").withIndex("by_scope_proposal", (q) =>
        q.eq("scopeKey", args.scopeKey).eq("proposalId", proposalId),
      ).collect(),
    ));
    ...
    const eventGroups = await Promise.all(aggregateRefs.map(({ type, id }) =>
      ctx.db.query("timelineEvents").withIndex("by_scope_aggregate", (q) =>
        q.eq("scopeKey", args.scopeKey).eq("aggregateType", type).eq("aggregateId", id),
      ).collect(),
    ));
...
export const cancelRun = mutation({
  ...
  returns: completionValidator,
  handler: async (ctx, args) => terminalizeRun(
    ctx,
    args,
    "cancelled",
    requireTrimmedText(args.reason ?? "Cancelled by request.", "reason"),
  ),
});
...
export const failRunSafely = mutation({
  ...
  handler: async (ctx, args) => terminalizeRun(
    ...
    args,
    "failed_safely",
    requireTrimmedText(args.reason ?? "Run failed safely.", "reason"),
  ),
});
...
export const getTimeline = query({
  ...
  handler: async (ctx, args) => {
    ...
    const events = await ctx.db
      .query("timelineEvents")
      .withIndex("by_scope_aggregate", (q) =>
        q.eq("scopeKey", args.scopeKey).eq("aggregateType", args.aggregateType).eq("aggregateId", args.aggregateId),
      )
      .order("asc")
      .take(limit);
